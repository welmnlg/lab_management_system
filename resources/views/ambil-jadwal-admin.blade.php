                ...
                <div class="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4">
                    <div class="flex-1">
                        ...
                        <div class="mb-4">
                            ...
                            <select id="building-select" class="w-full max-w-xs border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-sm">
                                <option value="">Loading gedung...</option>
                            </select>
                        </div>
                    ...
                    <div class="flex flex-col gap-3">
                        <!-- Tombol Kelola Gedung & Ruangan -->
                        <button id="kelola-gedung-btn" class="bg-[#1D518B] hover:bg-[#164376] text-white px-4 py-2 rounded-lg font-semibold transition-all duration-200 shadow-lg flex items-center justify-center space-x-2 text-sm w-full lg:w-auto">
                            <i class="fas fa-building"></i>
                            <span>Kelola Gedung & Ruangan</span>
                        </button>
...
<div id="kelola-gedung-modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden p-4" style="padding-top: 120px;">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-auto">
        <!-- Header Modal -->
        <div class="border-b border-gray-200 p-6">
            <h3 class="text-xl font-bold text-gray-800">Kelola Gedung & Ruangan</h3>
            <p class="text-gray-600 text-sm mt-1">Kelola data gedung dan ruangan laboratorium</p>
        </div>
        <!-- Content -->
        <div class="p-6">
            <div class="space-y-4">
                ...
                <div class="border border-gray-200 rounded-xl p-4">
                    <h4 class="font-semibold text-gray-800 mb-3">Tambah Gedung Baru</h4>
                    <form id="form-tambah-gedung" class="space-y-3">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Nama Gedung</label>
                                <input type="text" name="building_name" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm" placeholder="Contoh: Gedung C" required>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Kode Gedung</label>
                                <input type="text" name="building_code" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm" placeholder="Contoh: C" required>
                            </div>
                        </div>
                        <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium text-sm transition-all duration-200">
                            Tambah Gedung
                        </button>
                    </form>
                </div>
                <!-- Form Tambah Ruangan -->
                <div class="border border-gray-200 rounded-xl p-4">
                    <h4 class="font-semibold text-gray-800 mb-3">Tambah Ruangan Baru</h4>
                    <form id="form-tambah-ruangan" class="space-y-3">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Pilih Gedung</label>
                                <select name="building_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm" required>
                                    <option value="">Pilih Gedung</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Nama Ruangan</label>
                                <input type="text" name="room_name" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm" placeholder="Contoh: Lab Jaringan 1" required>
                            </div>
                        </div>
                        <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-medium text-sm transition-all duration-200">
                            Tambah Ruangan
                        </button>
                    </form>
                </div>
                <!-- Daftar Gedung dan Ruangan -->
                <div class="border border-gray-200 rounded-xl p-4">
                    <h4 class="font-semibold text-gray-800 mb-3">Daftar Gedung & Ruangan</h4>
                    <div id="daftar-gedung-ruangan" class="space-y-3">
        ...
        <div class="border-t border-gray-200 p-6 flex justify-end">
            <button type="button" id="tutup-kelola-gedung" class="px-4 py-2 text-gray-600 hover:text-gray-800 font-medium rounded-xl border border-gray-300 hover:border-gray-400 transition-all duration-200">
                Tutup
            </button>
        </div>
    </div>
...
.schedule-item .schedule-owner i {
    color: #9ca3af;
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const kelolaGedungBtn = document.getElementById('kelola-gedung-btn');
    const kelolaGedungModal = document.getElementById('kelola-gedung-modal');
    const tutupKelolaGedung = document.getElementById('tutup-kelola-gedung');
    ...
    // Modal Kelola Gedung
    if (kelolaGedungBtn) {
        kelolaGedungBtn.addEventListener('click', function() {
            loadBuildingsAndRooms();
            kelolaGedungModal.classList.remove('hidden');
        });
    }
    if (tutupKelolaGedung) {
        tutupKelolaGedung.addEventListener('click', function() {
            kelolaGedungModal.classList.add('hidden');
        });
    }
    // Modal Edit Jadwal
    if (batalEditJadwal) {
    ...
    [bukaSemesterModal, selesaikanSemesterModal, kelolaGedungModal, editJadwalModal, bukaPengambilanModal].forEach(modal => {
        if (modal) {
    ...
    function loadBuildingsAndRooms() {
        // Load buildings
        fetch('/api/buildings')
            .then(response => response.json())
            .then(result => {
                if (result.success) {
                    ...
                    // Load rooms for first building if exists
                    if (buildings.length > 0) {
                        loadRoomsForBuilding(buildings[0].building_id);
                    }
                }
            })
            .catch(error => {
    ...
    function populateBuildingSelects() {
        ...
        const buildingSelectModal = document.querySelector('select[name="building_id"]');
        if (buildingSelect) {
            buildingSelect.innerHTML = '<option value="">Pilih Gedung</option>';
            buildings.forEach(building => {
                buildingSelect.innerHTML += `
                    <option value="${building.building_id}">${building.building_name} (${building.building_code})</option>
                `;
            });
        ...
        if (buildingSelectModal) {
            buildingSelectModal.innerHTML = '<option value="">Pilih Gedung</option>';
            buildings.forEach(building => {
                buildingSelectModal.innerHTML += `
                    <option value="${building.building_id}">${building.building_name} (${building.building_code})</option>
                `;
            });
        }
        ...
        fetch(`/api/schedules/${scheduleId}`)
            ...
            .then(result => {
                ...
                if (result.success) {
                    ...
                    loadRoomsForEdit(schedule.room_id, schedule.building_name);
                } else {
    ...
    function loadRoomsForEdit(currentRoomId, buildingName) {
        console.log('⚙️ Loading rooms for building:', buildingName);
        fetch('/api/buildings')
            .then(response => response.json())
            .then(result => {
                if (result.success) {
                    let targetBuilding;
                    // Cari building berdasarkan nama
                    if (buildingName) {
                        targetBuilding = result.data.find(b => 
                            b.building_name === buildingName || 
                            buildingName.includes(b.building_code)
                        );
                    }
                    // Jika tidak ketemu, gunakan building pertama
                    if (!targetBuilding && result.data.length > 0) {
                        targetBuilding = result.data[0];
                    }
                    if (targetBuilding) {
                        fetch(`/api/buildings/${targetBuilding.building_id}/rooms`)
                            .then(response => response.json())
                            .then(roomResult => {
                            ...
                            .catch(error => {
                                console.error('❌ Error loading rooms:', error);
                                // Tetap tampilkan modal meski gagal load rooms
                                editJadwalModal.classList.remove('hidden');
                            });
                    } else {
                        // Tetap tampilkan modal meski tidak ada building
                        editJadwalModal.classList.remove('hidden');
                    }
                }
            })
            .catch(error => {
                console.error('❌ Error loading buildings:', error);
                // Tetap tampilkan modal meski gagal load buildings
                editJadwalModal.classList.remove('hidden');
            });
    }
