...
{
    /**
     * Get schedules for a specific room - FIXED VERSION
     */
    public function getSchedulesByRoom($roomId)
    {
        try {
            ...
            // Gunakan Eloquent dengan relationships untuk data yang lebih konsisten
            $schedules = Schedule::with(['user', 'course', 'class', 'room.building'])
                ...
                ->map(function($schedule) use ($currentUserId, $activePeriod) {
                    ...
                    Log::info('📋 SCHEDULE PERMISSION', [
                        ...
                        'schedule_user_name' => $schedule->user->name ?? 'Unknown',
                        'current_user_id' => $currentUserId,
                    ...
                    return [
                        ...
                        'user_id' => $scheduleUserId, // Pemilik jadwal (aslab)
                        'can_edit' => $canEdit
                    ];
                });
        ...
        try {
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    'day_of_week' => $schedule->day,
                    'time_slot' => $schedule->time_slot,
                    'user_id' => $schedule->user_id, // Pemilik jadwal (aslab)
                    'can_edit' => $canEdit
                ]
            ]);
        } catch (\Exception $e) {
    ...
    // ... (method lainnya tetap sama, tidak perlu diubah)
    // Kita hanya fokus pada getSchedulesByRoom dan show
    // Karena itulah endpoint yang digunakan oleh ambil-jadwal-admin.blade.php
    public function getUserCoursesActive()
    {
        try {
            ...
            // Get current semester type from active period
    ...
    public function store(Request $request)
    {
        try {
            ...
            // Cek apakah masih dalam masa pengambilan jadwal
            ...
            // Parse time slot to start_time and end_time
            ...
            // Check for user's own schedule conflicts
    ...
    public function update(Request $request, $scheduleId)
    {
        try {
            ...
            // Check for user's own schedule conflicts (exclude current schedule)
            ...
            if ($userConflict) {
                ...
                return response()->json([
                    'success' => false,
                    'message' => 'Dosen sudah memiliki jadwal lain pada waktu yang sama!'
                ], 422);
            }
    ...
    public function getAvailableTimeSlots(Request $request)
    {
        try {
            ...
            // Get booked time slots for the room and day
